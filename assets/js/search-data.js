...
ninja.data = [{
    ...
    handler: () => {
      window.location.href = "/";
        ...
        description: "Swizzle作用于SMEM的layout。给定layout范围内，Swizzle通过列异或操作（icol = irow ^ icol），周期性的coord重排，映射到新的物理地址offset。Swizzle定义了三个参数： $MBase$：以 $2^M$ 个一维坐标连续的元素为单位，将其当做一个元素； $SShift$：控制行号、列号提取的低位偏移； $BBits$：参与 XOR 的位数，即掩码位数，用于提取一维index中的行号、列号中的部分位。引用reed解释及图示，其输入为一个一维坐标的 layout，通过 swizzle 将其拆分为二维坐标表示形式： 📌 BBits表示有$2^B$个交换模式，SShift表示交换模式的周期。通常，$\mid{S}\mid \ge B$，如果$\mid{S}\mid \gt B$，则此交换模式重复$2^{\mid{S}\mid - B}$次，如果$\mid{S}\mid = B$，则只套用一次此交换模式。 📌 一般在设置Swizzle参数时，按输入的layout一行（准确的说是fast dimension）为周期进行swizzle，$2^{S+M}$ = 输入layout的列长度（此处仅指逻辑上的，实际完整的计算公式还需要考虑到元素存储字节数，具体见下面章Swizzle 参数设计规则）。比如 half 类型的 layout (8, 32):(32, 1)，定义swizzle&amp;lt;3, 3, 3&amp;gt;，即 8 个元素形成新的最小单位（M），8 个最小单位为一行（B），所以swizzle从$8 \times 8 = 64$个元素开始。见下面示例。B 为 8，则整个swizzle周期为 8 行。 设计Swizzle参数时，要求S &amp;gt;= B，即将掩码的源与目标分开（否则重合）。 上图借用reed博客中的图示，其理解S、B的作用与我理解的相反，放在此作为参考。 异或操作数学符号为 $\oplus$。1. Cute Swizzle 示例定义 layout...",
        section: "Posts",
        handler: () => {
        ...
        description: "tiled_copy.cu：官方示例1. Cute TiledCopy层次化的 copy 抽象，将 Copy_Atom 分为几个可组合的层次： CopyOperation：NVidia在不同的硬件架构、不同的存储层次之间数据搬运提供了不同的指令，如 ldmatrix 和 LDS 等，还有针对Ampere架构的 cp.async 等。 Copy_Traits：和 MMA_Traits 类似，提供了 CopyOperation 类型没有提供，但是其使用者 Copy_Atom 却需要的起到桥梁作用的信息； Copy_Atom：封装了基本的拷贝指令，针对 SRC-DST 的一次搬运；TiledCopy 则根据提供的 LayoutCopy_TV 执行 Copy_Atom，可能需要重复多次的 atom 搬运操作。1.1. Copy_AtomCopy_Atom 封装基本的拷贝指令，所以叫 Atom，即针对 SRC-DST 的一次搬运。适配不同的硬件指令集，比如通用拷贝/向量化 UniversalCopy&amp;lt;…&amp;gt;，cp.async（Ampere架构）。需要关注的两个模板参数是：CopyOperation 和 Copy_Traits。CopyOperation 定义了具体的拷贝指令，而 Copy_Traits 定义了拷贝的元信息，比如每次拷贝多少个元素（元素类型），SRC-DST 的 Layout 等。不同平台，实现不同的 Copy_Traits。（个人理解：一些 copy 操作也需要用到 layout 信息，以及 bits 位宽等信息）部分代码实现文件列表： cute/atom/copy_atom.hpp – Copy_Atom cute/atom/copy_traits.hpp – copy_unpack cute/atom/copy_traits.hpp – Copy_Traits...",
        section: "Posts",
        handler: () => {
        ...
        description: "测试用例：github – cuda_perf1. 启动配置使用Nsight Compute运行被测试CUDA程序，启动时，指定metrics为full：2. 分析Bank Conflicts被测试CUDA程序运行结束后，打开Nsight Compute的结果页面（Details），进入Memory Workload Analysis章节，在章节标题右侧，选择Memory Tables，查看Shared Memory部分：Details页面里显示的Shared Memory的Bank Conflicts信息，由于其测量数据来源于硬件计数器，还可能包括仲裁冲突(arbitration conflicts)： LDGSTS的Fill Return（全局/共享内存加载的返回数据） TMA的Fill Return（Tensor Memory Accelerator的返回数据） Tensor Core对共享内存的读取使用Source页面的L1 Wavefonts Shared Excessive结果，应该更准确：两个页面分析结果为什么会有差异？ 指标 数据来源 测量内容 用途 Details Page 硬件计数器 Bank Conflict + 仲裁冲突 实际性能影响 Source Page 代码分析 纯粹的 Bank Conflict 代码优化 如何识别可优化的Bank Conflict： 使用源页面（Source View）的Excessive计数器 这些是由地址发散和真实的Bank Conflict引起的 这些可以通过代码优化来消除哪些冲突无法修复： 详情页面中显示的仲裁冲突无法通过代码优化解决 这些是系统级别的问题（Tensor Core、TMA、全局内存的更高优先级访问） 这些是硬件行为的自然结果3. 参考资料 Questions on L1...",
        section: "Posts",
        handler: () => {
        ...
        description: "现代性能分析，使用针对pipeline的分析办法(取代CPU cycles分析)。这源于现代CPU架构的复杂性。现代CPU处理指令架构，分为前端 Front-end，后端 Back-end两部分。阻碍指令执行的因素，从硬件看，源于前端或后端的Stall。1. CPU 流水线Intel CPU流水线一般分为5级。其中解码(ID)，意思是将指令操作分解为多个uOp(即拆分为多个更低级的硬件操作)，如ADD eax, [mem1]，可以拆分成两个微指令：从内存读取数据，再执行ADD操作。2. CPU 架构及流水线的执行过程执行过程：前端执行完IF -&amp;gt; ID之后，然后在一个名为allocation的过程中，uOps被输送到后端。后端监控uOp的操作数(data operand)何时可用，并在可用的执行单元中执行uOps。当uOp执行完成之后，称之为执行完成(retirement)，且uOp的结果被写会寄存器或者内存。大多数uOps都会完全通过流水线并退出，但有些投机指令uOps可能会在退出前被取消–如预测错误的分支。在处理器架构中，有一个抽象概念：pipeline slot，即执行端口。在Intel处理器中，一个core一般有四个执行端口，即每个cycle最多可以执行四个uOps。VTune在allocation阶段，可以测量pipeline slot的利用率(星号标注的地方)。3. Top-down 分析方法从性能分析的角度看，一条微指令在流水线中的性能指标可以分为： 退出(Retiring) – Micro Sequencer(微指令调度器)可能会成为瓶颈，例如调度浮点指令。 分支预测错误(Bad Speculation) – 分支预测错误，或者memory ordering violation(多核多线程共享数据情形)，导致Machine Clears(清除流水线)。 前端瓶颈(Front-End Bottleneck) 后端瓶颈(Back-End Bottleneck)顶级分类分析步骤：3.1 Frontend Bound – 前端瓶颈前端主要职责为读取指令，解码之后，发送给后端。遇到分支指令，需要经过预测器预测下一个指令的地址，这意味着会出现由于分支预测错误导致的ICache Miss而引起前端阻塞。3.2 Back-End Bound – 后端瓶颈后端主要职责为执行指令，包括ALU、FPU、Memory等。后端瓶颈分为： Core Bound Memory Bound如D Cache Miss，浮点除法器过载，指令依赖、数据依赖等。4. 一些优化手段4.1 Frontend 减少代码的footprint，如-fomit-frame-pointer 调整代码布局：如是用-fprofile-generate -fprofile-use 调整代码布局：如用__attribute__((hot)) 分支预测优化：如loop unrolling，特别是小的循环，如小于64次循环 分支预测优化：是用if代替三目运算符；避免if-elses结构，switch-case排序4.2 Back-End 减少function call，如inline...",
        section: "Posts",
        handler: () => {
