...
ninja.data = [{
    ...
    handler: () => {
      window.location.href = "/";
        ...
        description: "Swizzle作用于SMEM的layout。给定layout范围内，Swizzle通过列异或操作（icol = irow ^ icol），周期性的coord重排，映射到新的物理地址offset。Swizzle定义了三个参数： $MBase$：以 $2^M$ 个一维坐标连续的元素为单位，将其当做一个元素； $SShift$：控制行号、列号提取的低位偏移； $BBits$：参与XOR的位数，即掩码位数，用于提取一维index中的行号、列号中的部分位。引用reed解释及图示，其输入为一个一维坐标的layout，通过swizzle将其拆分为二维坐标表示形式：BBits表示有$2^B$个交换模式，SShift表示交换模式的周期。通常$\mid{S}\mid \ge B$，如果$\mid{S}\mid \gt B$，则此交换模式重复$2^{\mid{S}\mid - B}$次，如果$\mid{S}\mid = B$，则只套用一次此交换模式。一般在设置Swizzle参数时，按输入的layout一行（准确的说是fast dimension）为周期进行swizzle，$2^{S+M}$ = 输入layout的列长度（此处仅指逻辑上的，实际完整的计算公式还需要考虑到元素存储字节数，具体见下面章Swizzle 参数设计规则）。比如 half 类型的layout (8, 32):(32, 1)，定义swizzle&amp;lt;3, 3, 3&amp;gt;，即 8 个元素形成新的最小单位（M），8 个最小单位为一行（B），所以swizzle从$8 \times 8 = 64$个元素开始。见下面示例。B 为 8，则整个swizzle周期为 8 行。 设计Swizzle参数时，要求S &amp;gt;= B，即将掩码的源与目标分开（否则重合）。 上图借用reed博客中的图示，其理解S、B的作用与我理解的相反，放在此作为参考。 异或操作数学符号为 $\oplus$。1. Cute Swizzle 示例定义 layout (8, 32):(32, 1)，定义swizzle&amp;lt;3, 2, 3&amp;gt;，即 $2^{3+2}$ = 32（输入layout列宽）。代码如下：from cutlass import...",
        section: "Posts",
        handler: () => {
