...
# 在 Jetson Orin AGX 上挂载 M.2 SSD 并用于 Docker 与用户数据
...
## 3. 把 Docker 镜像/容器数据放到 SSD

### 推荐方案：修改 Docker 的 `data-root`
...
#### 6.4.1 删除 Docker 原始目录的残留（如果仍然占用明显空间）
...
#### 6.4.3 可选：清理 Docker 未使用的镜像/缓存（进一步释放 SSD 空间）
