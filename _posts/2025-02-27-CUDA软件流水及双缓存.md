...
CTA loop-over BK 分块公式：

$$
C[i, j] = \sum_{k=0}^{nBK-1} A[i, k] \times B[k, j]
$$

整个 GEMM 计算可以表达为：

$$
C[i, j] = \sum_{k'=0}^{nK-1} \sum_{k=0}^{nBK-1} A[i, k] \times B[k, j]
$$

图示如下（引用自<https://am17an.bearblog.dev/a-gentle-introduction-to-gemm-using-mma-tensor-cores/>）：

![gemm_formula](/assets/images/cuda/20250227/gemm_tiled_mma_tiled_copy_pipeline/23-1.webp)
...
在 GEMM 分层结构的每一层都使用双缓冲：使得上游流水段在写入共享内存或寄存器时，下游依赖段也可以从其存储体读数据。值得注意的是，这样可以省去第二个 `__syncthreads()`，因为一个共享内存缓冲被写、另一个被读。
