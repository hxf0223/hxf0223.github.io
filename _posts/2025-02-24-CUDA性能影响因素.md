...
* 数据重用（`global memory`）：例如将大块内存数据加载到`shared memory`，以减少对`global memory`的访问次数，从而提高`AI`。
* `Bank Conflicts`（`shared memory`）：比如矩阵计算`C = A * B`中的共享内存访问冲突，将`B`矩阵加载到`shared memory`之后，进行转置，避免`Bank Conflicts`。
