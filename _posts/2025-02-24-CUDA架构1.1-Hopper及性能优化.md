...
> 📝在`Source Page`页面中，查看瓶颈的对应的代码/汇编，比如：
>
> - `Compute Bound`：查看`Instructions Executed`（指令执行栏），并通过菜单栏定位到占比最多的指令；
> - `Memory Bound`：查看`L1 Tag Requests Global`/`L1 Wavefronts Shared`/`L2 Theotetical Sectors Global`等栏目；
> - `Latency Bound`：查看`Warp Stall Samplining(Not issued Samplies)`栏目；

![scoreboard](/assets/images/cuda/20250224/cuda_arch_01/scoreboard_dependencies.png)

> 📝对一些Stall比较长的计算指令，使用`Scoreboard Dependencies`查看其依赖的指令和资源，从而分析瓶颈原因。（`Scoreboard`即记分牌，有专门的硬件记录指令的状态，比如在等待从内存加载数据）
...
- 指令延迟，如指令依赖，或者分支发散严重（此时`issue Slot Utilization`较低）；
...
| L1 Hit Rate                  | 过低                   | 数据局部性差，考虑 tiling / \_\_ldg()           |
...
- [HOW TO UNDERSTAND AND OPTIMIZE SHARED MEMORY ACCESSES USING NSIGHT COMPUTE(S41723)(pdf)](/assets/pdf/cuda/How_to_Understand_and_Optimize_Shared_Memory_Accesses_using_Nsight_Compute_1647052898718001rNP8.pdf)
...
- [GTC Silicon Valley-2019: Performance Analysis of GPU-Accelerated Applications using the Roofline Model](https://developer.nvidia.com/gtc/2019/video/s9624)
- [GTC Silicon Valley-2019: Performance Analysis of GPU-Accelerated Applications using the Roofline Model(pdf)](/assets/pdf/cuda/s9624-performance-analysis-of-gpu-accelerated-applications-using-the-roofline-model.pdf)
