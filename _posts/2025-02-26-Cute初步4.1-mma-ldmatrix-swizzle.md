...
### 3.4. ldmatrix 与 ldmatrix.trans

区别在于，加载到寄存器之后，数据布局不同（也可以看作线程布局不同）。总结如下表：

| 特性           | ldmatrix             | ldmatrix.trans                           |
| -------------- | -------------------- | ---------------------------------------- |
| 每线程指针指向 | 共享内存中的一行     | 共享内存中的一列                         |
| 寄存器元素宽度 | uint32_t（2个 FP16） | uint16_t × 2（同样2个 FP16，但布局不同） |
| 适用矩阵       | A 矩阵（行主序）     | B 矩阵（列主序存储时）                   |

![mma_ldmatrix_trans_relation](/assets/images/cuda/20250226/cute_ldmatrix_mma_swizzle/mma_ldmatrix_trans_relation.png)

**ldmatrix.trans**用于矩阵 B 以列主序存储在共享内存时，硬件在加载过程中完成转置，使寄存器中的布局直接匹配 mma.sync 对 B 操作数的期望格式。

测试代码：<https://github.com/HPC02/cuda_perf/blob/master/src/study_codes/study_tests/test_tiled_copy.cu>，其中的测试用例 TiledCopy::LdmatrixX1_N_vs_T。

### 3.5. tiled mma 及 smem tiled copy
...
### 3.6. 有关`Inverse TV-Layout`图示的说明
