...
- `m_zzz_msk`没有参与`swizzle`计算，在`CuTe`库中用作检查作用。
...
针对每个`offset`，经过`swizzle`映射之后，异或更新低位掩码对应的值：
...
以一个 `layout (32, 16):(16, 1)`为例，分析`swizzle<B=4, M=0, S4>` 参数变动对结果的影响规律。

经过`offset & yyy_mk`提取行号低四位（以及`shiftr`操作得到最终行号提取掩码），得到第 0 行、第 16 行（0x10）由于行号掩码提取过后的低 4 位为 0，导致`swizzle`无效。
...
创建一个列主序的 `Layout (32, 32):(1, 32)`，`XOR`此时作用在行号上（即 `fast dimension`），但是从物理存储上看，其结果与行主序的 `layout` 是相同的。这个列主序 `Layout` 原始图示，以及 `Swizzle` 映射之后图示如下：

![origin_column_major_32_32_505](/assets/images/cuda/20250226/cute_swizzle/original_layout32x32_column_major.svg)
![swizzle_32_32_505](/assets/images/cuda/20250226/cute_swizzle/swizzled_layout32x32_column_major_505.svg)

另外，`Swizzle`就是一个复合映射，演示代码如下：
...
> 即要求 32 个连续的 word 地址，经过 `swizzle` 之后，分别落入 `shared memory` 的`32`个`bank`中。
...
> `Thread block swizzle`，以及 `block` 调度顺序，见 github issue：[[QST]how to understand "block swizzling"](https://github.com/NVIDIA/cutlass/issues/1017)，以及博客[Nvidia Tensor Core-CUDA HGEMM Advanced Optimization](https://bruce-lee-ly.medium.com/nvidia-tensor-core-cuda-hgemm-advanced-optimization-5a17eb77dd85)。
...
- [CUTLASS CuTe GEMM细节分析（四）——谈谈Swizzle模板参数中关于B和S的一些误区](https://zhuanlan.zhihu.com/p/1949609885953139032)：知乎 Anonymous 文章
- [淺談CUTLASS / CuTe的Swizzling Functor](https://hackmd.io/@chrischang80/H1CXHWS-Je)
...
- [NVIDIA Ampere GA102 GPU Architecture Whitepaper](https://www.nvidia.com/content/PDF/nvidia-ampere-ga-102-gpu-architecture-whitepaper-v2.pdf)
